...
void main() {
  group('EOS Model', () {
    ...
    test('Block header state to JSON', () {
      ...
      expect((headerState.activeSchedule?.producers?.length ?? 0) > 10, isTrue);
      ...
      expect(
          (headerState.blockrootMerkle?.activeNodes?.length ?? 0) > 10, isTrue);
      expect(headerState.blockrootMerkle?.activeNodes?[0],
          '02a5e503fa636e7a35d9fa04df9c2ab7860864acff3d835b6c1ec1259b7867e2');
      expect(headerState.blockrootMerkle?.nodeCount, 44426499);
      expect((headerState.producerToLastProduced?.length ?? 0) > 10, isTrue);
      expect((headerState.producerToLastImpliedIrb?.length ?? 0) > 10, isTrue);
      expect(headerState.blockSigningKey,
          'EOS4wZZXm994byKANLuwHD6tV3R3Mu3ktc41aSVXCBaGnXJZJ4pwF');
      expect((headerState.confirmCount?.length ?? 0) > 10, isTrue);
    });
  });
}
